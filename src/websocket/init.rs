...
pub async fn web_socket_init(
    ...
    stdin_tx
        .send(Message::Text(parse_token.clone()))
        .await
        .unwrap();
    stdin_tx
        .send(Message::Text(String::from("NICK thatgirlwhocodes")))
        .await
        .unwrap();
    //FIXME: Remove mut keyword and manual channel joins
    let mut channel_list = get_channels_to_join(&pool).await;
    channel_list.push(String::from("alienoro"));
    channel_list.push(String::from("thatgirlwhocodes"));
    tokio::spawn(loop_through_channels_to_join(
        stdin_tx.clone(),
        channel_list,
    ));
...
pub async fn leave_channel(stdin_tx: async_channel::Sender<Message>, channel: String) {
    stdin_tx
        .send(Message::Text(format!("PART #{}", channel)))
        .await
        .unwrap();
}
